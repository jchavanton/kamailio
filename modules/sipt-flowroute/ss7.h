...
#define IAM_NO_BODY       -1   /* There is no IAM body */
#define IAM_NOT_IAM       -2   /* The message is not actually IAM */
#define IAM_OVERFLOW      -3   /* The buffer is too small to contain the IAM */
#define IAM_MISSING_PARAM -4   /* A required parameter is not present */
#define IAM_BAD_POINTER   -5   /* A parameter pointer points somewhere bad */
#define IAM_BAD_TERMINAL  -6   /* The final byte of the IAM is not 0 */
#define IAM_UNDERFLOW     -7   /* The buffer contains trailing bytes */
...
typedef struct isup_parm_opt {
	unsigned char type;
	unsigned char len;
	unsigned char data[0];
} isup_parm_opt_t;

typedef struct isup_iam_fixed {
	unsigned char type;
	...
	/* TODO:
	 * The original implementation (from Kamailio 4.1) assumed the
	 * following structure at this position within IAM:
	 *
	 *   unsigned char transmission_medium_req; // Mandatory, fixed
	 *   unsigned char called_party_ptr;        // Mandatory, variable pointer
	 *
	 * Thus, sipt assumed that there was just a single mandatory, variable
	 * length field (called_party_number) immediately after the optional
	 * part pointer. This is apparently only true for ITU networks; ANSI
	 * networks use the following members:
	 *
	 *   unsigned char user_service_ptr;       // Mandatory, variable pointer
	 *   unsigned char called_party_ptr;       // Mandatory, variable pointer
	 *
	 * In such messages, the data immediately following the optional pointer
	 * is the user_service_information, *not* the called party number. So
	 * sipt was getting these horribly wrong.
	 *
	 * Currently, we have no idea if the ITU or ANSI formats are indicated
	 * anywhere in the IAM fixed part. We'll want to look into this more
	 * later, if we ever care.
	 *
	 * More information:
	 *    http://www.itu.int/rec/T-REC-Q.763/en
	 */
	unsigned char user_service_ptr;
	unsigned char called_party_ptr;
	unsigned char optional_pointer;
	unsigned char variable_data[0];
} isup_iam_t;
