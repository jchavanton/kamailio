...
void rms_media_destroy(call_leg_media_t *m) {
	LM_NOTICE("rtp_session_destroy[%p]\n", m->rtps);
	rtp_session_destroy(m->rtps);
	m->rtps=NULL;
	LM_NOTICE("ms_ticker[%p]\n", m->ms_ticker);
	ms_ticker_destroy(m->ms_ticker);
	m->ms_ticker=NULL;
	LM_NOTICE("ms_factory_destroy[%p]\n", m->ms_factory);
...
int create_call_leg_media(call_leg_media_t *m){
	...
	LM_NOTICE("RTP session [%s:%d]<>[%s:%d]\n", m->local_ip.s, m->local_port, m->remote_ip.s, m->remote_port);
	m->rtps = ms_create_duplex_rtp_session(m->local_ip.s, m->local_port, m->local_port+1, ms_factory_get_mtu(m->ms_factory));
	rtp_session_set_remote_addr_full(m->rtps, m->remote_ip.s, m->remote_port, m->remote_ip.s, m->remote_port+1);
...
int rms_stop_media(call_leg_media_t *m) {
	...
	if (m->ms_resampler) ms_connection_helper_unlink(&h, m->ms_resampler,0,0);
	...
	if (m->ms_resampler) ms_filter_destroy(m->ms_resampler);
