...
int mqueue_db_load_queue(str *name)
{
	...
	if (DB_CAPABILITY(mq_dbf, DB_CAP_FETCH)) {
		if(mq_dbf.query(mqueue_db_con,0,0,0,db_cols,0,ncols,0,0) < 0)
		{
	...
	do {
		for(i=0; i<RES_ROW_N(db_res); i++)
		{
			...
			cnt++;
			LM_INFO("adding item[%d] key[%.*s] value[%.*s]\n", cnt, key.len, key.s, val.len, val.s);
...
int mqueue_db_save_queue(str *name)
{
	...
	for(i=0;i<mqueue_sz;i++) {
		...
		key = get_mqk(name);
		val = get_mqv(name);
		LM_INFO("inserting mqueue[%.*s] name[%.*s] value[%.*s]\n",
	...
	LM_INFO("save the content of queue [%.*s]\n",
			name->len, name->s);
	mqueue_db_close_con();
	return 0;
error:
