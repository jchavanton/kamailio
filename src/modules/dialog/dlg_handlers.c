...
void extra_ack_debug_info(const char *s, struct sip_msg* req) {
	...
	LM_INFO("[ACK] %s >>> call-id[%.*s] to-tag[%.*s]\n", s, req->callid->body.len, req->callid->body.s, req->to->body.len, req->to->body.s);
}
...
void dlg_onroute(struct sip_msg* req, str *route_params, void *param)
{
	...
	if ( seq_match_mode!=SEQ_MATCH_NO_ID ) {
		if( d_rrb.get_route_param( req, &rr_param, &val)!=0) {
			...
			if (seq_match_mode==SEQ_MATCH_STRICT_ID ) {
				extra_ack_debug_info("seq_match_mode STRICT_ID (#1)", req);
				return;
			}
		} else {
			...
			if (dlg==0) {
				...
				if (seq_match_mode==SEQ_MATCH_STRICT_ID ) {
					extra_ack_debug_info("seq_match_mode STRICT_ID (#2)", req);
					return;
				}
			} else {
				...
				if (match_dialog( dlg, &callid, &ftag, &ttag, &dir )==0) {
					...
					if (seq_match_mode==SEQ_MATCH_STRICT_ID ) {
						extra_ack_debug_info("seq_match_mode MATCH_STRICT_ID (#3)", req);
						return;
					}
				}
			}
	...
	if (dlg==0) {
		...
		if (dlg==0){
			...
			extra_ack_debug_info("get_dlg not found", req);
			return;
		}
	}
	...
	if (new_state != DLG_STATE_CONFIRMED) {
		extra_ack_debug_info("state not changed", req);
